<template>
  <VCard>
    <ClientOnly>
      ...
      <div
        class="flex flex-row flex-wrap p-2 gap-2 text-xs items-center"
        v-else-if="legendEntries.length || absences !== 'off'"
      >
        <div
        ...
        >
          <div
            :class="['w-3', 'h-3', 'm-1', 'rounded-sm', entry.background]"
            :style="entry.color ? { backgroundColor: entry.color } : null"
          />
...
import { keywordLabel } from './utils'
...
const legendEntries = computed(() => {
  const keywords = store.distribution.currentKeywords
  const entries = []

  store.distribution.currentShapeTypes.forEach((type) => {
    if (type === ASSERTED_DISTRIBUTION && keywords.length) return

    entries.push({
      key: type,
      label: t(LEGEND[type].labelKey),
      background: LEGEND[type].background,
      color: null
    })
  })
  ...
  keywords.forEach((keyword) => {
    entries.push({
      ...
      label: keyword.labelKey
        ? t(keyword.labelKey)
        : keywordLabel(keyword, locale.value),
      background: keyword.color ? null : LEGEND.AssertedDistribution.background,
      color: keyword.color
    })
  })
